...
🚧 **M1: MVP ほぼ完了** — 2ペインの実ファイル一覧 + hjkl ナビ、隠しファイル切替(Ctrl+H)、ペイン間コピー/移動(F5/F6)・削除(Delete/Shift+Delete)・リネーム(F2)・新規フォルダ(F7)を**安全モードゲート付き**で実装。同名衝突は**名前変更(任意入力)/上書き/キャンセル**の3択。**お気に入り**(ネスト・検索・Ctrl+D追加)、ヘルプ `?`/`F1`、テーマ・文字サイズ設定も実装済み。次: 仕上げ → M2(Places/タブ/セッション復元)。
...
- [WSL 連携 (docs/WSL.md)](docs/WSL.md) — WSL 上の Tana から Windows 側のアプリ / エクスプローラーで開く
