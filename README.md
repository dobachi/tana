...
🚧 **M1: MVP 実装中** — M0 完了。2ペインの実ファイル一覧 + hjkl ナビ、隠しファイル切替(Ctrl+H)、ペイン間コピー/移動(F5/F6)・削除(Delete=ゴミ箱 / Shift+Delete=完全削除)を**安全モードゲート付き**で実装済み。同名衝突時は**名前変更(インクリメント)/上書き/キャンセル**の3択。次: リネーム(F2)・新規フォルダ(F7)・お気に入り。
