...
- ペイン間操作の基本動作:
  ...
  - **リネーム/新規フォルダ**: インライン編集
- **同名衝突時 (NFR-R3)**: 「**名前を変えてコピー(インクリメント)** / **上書き** / **キャンセル**」の3択モーダル(`core/conflictdialog.js`)。インクリメント名は backend `unique_name` が `name (1).ext`, `name (2).ext` … を採番。上書き・名前変更とも操作モードでのみ実行(安全モードでは抑止)
