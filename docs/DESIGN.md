...
> 以下は**目標形**。現状の実体ファイル構成は [STATUS.md §3 コードマップ](STATUS.md#3-コードマップ現状の実体) を参照（まだ `core/` に集約、Rust は `lib.rs` 単一）。
...
- ペイン間操作の基本動作:
  ...
  - **リネーム(`F2`)/新規フォルダ(`F7`)**: 名前入力モーダル(`core/inputdialog.js`)。既定値はリネーム時=現在名(拡張子前を選択)、新規=「新しいフォルダ」。同名があれば通知
- **同名衝突時 (NFR-R3)**: 「**名前を変えて実行** / **上書き** / **キャンセル**」の3択モーダル(`core/conflictdialog.js`)。名前は**テキスト入力で任意指定可**、既定値は backend `unique_name` が採番したインクリメント名(`name (1).ext`, `name (2).ext` …)。入力名も衝突したら再プロンプト。上書き・名前変更とも操作モードでのみ実行(安全モードでは抑止)
  - **同じフォルダへのコピー/移動**(両ペインが同一ディレクトリ)も衝突として扱い、同モーダルを表示。「名前を変えて実行」で複製になる。自分自身への上書きは安全のため拒否(明確なメッセージ)
...
- **実装**: モデル `core/favorites.js`(真実源・`localStorage` 永続化)/ ビュー `core/favoritesview.js`(サイドバー描画)。`Ctrl+D` で現在のフォルダを追加、`＋` でフォルダ作成、行ダブルクリックで名前変更、`×` で削除、ブックマーククリックでアクティブペインに開く。お気に入り編集は設定操作なので**安全モードでも可**(ファイル破壊ではない)
- 並べ替え・D&D による再編成は将来(操作モードを要するかは「お気に入りは設定でありファイル破壊ではない」ため安全モードでも許可とする方針)
...
- **外部アプリ連携**: 起動は `@tauri-apps/plugin-opener` に任せる(自前の Rust コマンドは持たない)
  - 既定アプリで開く / ファイルマネージャで表示: `openPath(path)` / `revealItemInDir(path)`
  - **別のアプリで開く**: `openPath(path, command)`。7-Zip 等のよく使う外部ツールを**ユーザー定義コマンド**として `core/extapps.js`(真実源・`localStorage`)に登録し、設定画面で管理する
  - 呼び出し口は3つ: 右クリック →「別のアプリで開く…」(登録一覧 + その他… + 管理)、二打鍵 `o → 1..9`(登録の上から9件)、`o → a`(コマンドを都度入力)
  - `menu.js` はサブメニューを持たないので、「別のアプリで開く…」は同じ位置に**一段掘り下げたメニューを開き直す**(キーボード操作は `showMenu` のまま使える)
  - 対象は**カーソル行の1ファイル**(既存の「外部アプリで開く」と同じ)。複数選択への一括適用は行わない
- 渡せるのは**プログラム1つだけで引数は付けられない**(`code --wait` は不可)。OS ごとに意味も異なる:
  Linux `Command::new(cmd).arg(path)` / macOS `open -a <cmd> <path>` / Windows `cmd /c start "" "<cmd>" "<path>"`。
  Windows は引用符がエスケープされずに渡るため、`"` を含むコマンドは `extapps.js` 側で拒否する
- ACL: `openPath` にアプリを渡すには capability の scope で `app` を許可する必要がある
  (`{"path": "**", "app": true}`)。省略時は既定アプリのみ許可で、アプリ指定は実行時に弾かれる。
  付け忘れは単体テストのモックでは出ないので `__tests__/capabilities.test.js` で静的に突き合わせる
...
| お気に入りに現在地を追加                    | `Ctrl+D`                                | 常時                   |
| 文字サイズ 拡大 / 縮小 / リセット           | `Ctrl++` / `Ctrl+-` / `Ctrl+0`          | 常時                   |
| ヘルプ（ショートカット一覧）表示/閉じる     | `?` / `F1`（`Ctrl+?` も可）             | 常時                   |
