...
- **スナップショット日**: 2026-07-23
- **基準コミット**: `8da2512 release: v0.4.4`（`origin/main` と同期済み）に D&D (FR-02/FR-11) を追加
- **現在のフェーズ**: **M1 (MVP) 完了圏** → 仕上げ → M2 着手前
...
| FR-10 | 全機能キーボード到達 | M | 🟡 | 主要操作は到達可。メニューバーに「編集」を追加し、コピー/移動/リネーム/削除/新規フォルダをマウス・キーボード両方から到達可能に。網羅性の最終点検は残 |
...
> **注意**: 優先度 **M** の未達は FR-10（キーボード到達性の点検）のみになった。M1 完了を宣言するには、この点検と、D&D の `Shift`+ドロップのネイティブ実機確認（§7）が残っている。
...
1. **D&D のネイティブ実機確認**: `make dev` で起動し、`Shift`+ドロップが移動になること、フォルダ行への吸い込み、複数選択のドラッグを目視する。noVNC では修飾キーが届かないので、ここだけは実機が要る（§6）。
2. **FR-10 キーボード到達性の点検**: 優先度 **M** で唯一残った 🟡。全機能がキーボードから到達できるか棚卸しし、抜けを埋める。D&D で足した経路（ドロップ先の指定）にキーボード等価物が要るかもここで判断する。
3. **M1 完了の宣言**: 1・2 が済んだら REQUIREMENTS/README と突き合わせて M1 を閉じる。
