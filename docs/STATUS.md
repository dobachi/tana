...
- **基準コミット**: `4343d57 release: v0.4.6`（`origin/main` と同期済み）に FR-10 キーボード到達性の仕上げを追加
- **現在のフェーズ**: **M1 (MVP) 完了圏** → 仕上げ（FR-10 点検済み）→ 残るは D&D ネイティブ実機確認のみ → M2 着手前
...
テストは `src/js/__tests__/<name>.test.js` に対応（32ファイル）。
...
| FR-02 | ペイン間コピー/移動 | M | ✅ | F5/F6（複数選択対応）＋ D&D。既定コピー / Shift で移動 |
...
| FR-10 | 全機能キーボード到達 | M | ✅ | 網羅性点検を実施（2026-07-27）。コンテキストメニューを `Shift+F10`/`≣` で開いたとき先頭項目へフォーカスする（マウスと同じ即操作性）。同メニュー専用の「ファイルマネージャで表示・パス/名前コピー」の開き方をヘルプに明記。残る非到達は D&D の「同ペイン内フォルダへのドロップ」のみで、これはマウス専用の利便機能（別ペインへのコピー/移動は F5/F6 で到達可）と判断 |
...
> **注意**: 優先度 **M** の未達は解消（FR-10 点検完了）。M1 完了を宣言するために残るのは、D&D の `Shift`+ドロップのネイティブ実機確認（§7・§6）のみ。これは noVNC では修飾キーが届かず検証できないため、`make dev` のネイティブ起動での目視が要る。
...
- **D&D の `Shift`+ドロップ（移動）が実機未確認**。noVNC 越しでは RFB の PointerEvent に修飾キーの欄が無く、修飾キー付きの操作が「修飾なし」として届くため、`make docker-gui` では検証できない。**ネイティブ起動での確認が必要**（→ [DRAG-AND-DROP.md](DRAG-AND-DROP.md) §5「検証環境の制約」）。判定ロジック自体は単体テストで押さえてある。
...
1. ~~**FR-10 キーボード到達性の点検**~~ ✅ 完了（2026-07-27）。全操作を棚卸しし、キーボードで開いたコンテキストメニューの先頭フォーカスと、メニュー専用操作のヘルプ明記で抜けを埋めた。D&D の「同ペイン内フォルダへのドロップ」だけはマウス専用の利便機能と判断（F5/F6 で別ペイン到達可）。
2. **D&D のネイティブ実機確認**: `make dev` で起動し、`Shift`+ドロップが移動になること、フォルダ行への吸い込み、複数選択のドラッグを目視する。noVNC では修飾キーが届かないので、ここだけは実機が要る（§6）。**M1 完了宣言に残る唯一の手動ゲート**。
3. **M1 完了の宣言**: 2 が済んだら REQUIREMENTS/README と突き合わせて M1 を閉じる。
4. M2 着手: **FR-07 Places 検出**（OS別、`places` モジュール新設 + Rust 側 `places.rs` 切り出し）→ **FR-08 タブ**。Q4（お気に入りの永続化を localStorage から設定ディレクトリJSONへ移すか）はタブのセッション復元と絡むのでここで決める。
