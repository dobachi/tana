...
| FR-02 | ペイン間コピー/移動 | M | 🟡 | F5/F6 実装済。D&D は未 |
...
> **注意**: README は「FR-13 コンテキストメニュー」を M1 範囲に含めているが、現状コードには未実装。M1 を「完了」と宣言する前に FR-02(D&D)/FR-11/FR-13/FR-14 の扱いを確定すること（下記「次の一手」参照）。
...
- **D&D 未実装**（FR-02/FR-11）。現状コピー/移動は F5/F6 キーのみ。
- **コンテキストメニュー未実装**（FR-13）。右クリック / Shift+F10 からの操作が無い。
- **セッション復元未実装**（FR-14）。終了時のディレクトリ/ペイン状態が次回起動に引き継がれない。
