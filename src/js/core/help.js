...
export const SHORTCUTS = [
  {
    ...
    items: [
      ['j / k', 'カーソル 下 / 上'],
      ['h', '親フォルダへ戻る'],
      ['l / Enter', '開く（フォルダは中へ / ファイルは既定アプリで）'],
      ['Alt + ← / →', '履歴を戻る / 進む（マウスの戻る/進むボタンでも可）'],
      ['g / G', '先頭 / 末尾へ'],
      ['Tab', 'ペイン切替（左 ⇄ 右）'],
      ['Ctrl + T / W', 'タブを開く / 閉じる（アクティブペイン）'],
      ['Ctrl + Tab / Ctrl+Shift+Tab', '次 / 前のタブ'],
      ['t → h / l', 'アクティブタブを左 / 右へ移動（並べ替え）'],
      ['y → p / n / d', 'コピー: パス / 名前 / 現在地のパス'],
      ['o → o / r', '既定のアプリで開く / ファイルマネージャで表示'],
      ['o → 1…9', '設定に登録した外部アプリで開く（設定 → 外部アプリ）'],
      ['o → a', '別のアプリで開く（コマンド名を都度入力）'],
      ['o → w / e', 'WSL: Windows の既定アプリで開く / エクスプローラーで表示'],
      ...
      [
        'Shift + F10 / ≣',
        'コンテキストメニュー（右クリック相当。ファイルマネージャで表示・パス/名前のコピー等）',
      ],
    ],
  },
  {
    ...
    items: [
      ['F5', '別ペインへコピー'],
      ['F6', '別ペインへ移動'],
      ['Ctrl + C', 'コピー登録（任意の現在地へ Ctrl+V で貼り付け）'],
      ['Ctrl + X', '切り取り登録（Ctrl+V で移動貼り付け・一度きり）'],
      ['Ctrl + V', 'アクティブペインの現在地へ貼り付け'],
      ['F2', '名前の変更'],
      ['F7', '新しいフォルダ'],
      ['Delete', 'ゴミ箱へ削除'],
      ['Shift + Delete', '完全に削除'],
  ...
  },
  {
    ...
    items: [
      ['Ctrl + D', '現在のフォルダを追加'],
      ['Ctrl + B', 'サイドバー巡回: ペイン → 場所 → お気に入り → ペイン'],
      ['Ctrl + L', 'パスを直接入力して移動（~ 展開・相対パス可）'],
      ['/ または Ctrl + F', '現在ディレクトリ内を検索（名前・本文, Esc で閉じる）'],
      ['（一覧で）j / k', '上 / 下'],
  ...
  },
  {
    ...
    items: [
      ...
      ['Ctrl + R', '再読込（外部変更は自動でも反映される）'],
      ['Ctrl + Shift + B', 'メニューバーを隠す/戻す'],
      ['Alt', 'メニューバーを開く（Alt+F / V / H で各メニュー、↑↓←→で移動）'],
      [
        's → n / s / m / e',
        '並び替え: 名前 / サイズ / 更新日時 / 拡張子（s→r で反転。列ヘッダのクリックでも可）',
      ],
      ['Ctrl + ,', '設定を開く（テーマ切替はここ、または表示メニュー）'],
      [
        'Ctrl + + / - / 0',
        '文字サイズ 拡大 / 縮小 / リセット（Ctrl+ホイール、ステータスバーの A- / A+ でも可）',
      ],
    ],
  },
  {
