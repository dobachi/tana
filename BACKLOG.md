...
- [ ] 2026-07-27 現在ディレクトリ内 grep（内容/ファイル名検索）: いま開いているディレクトリ配下を再帰検索する機能。Rust バックエンドで走査（テキスト内容マッチ＋ファイル名マッチ、隠し/バイナリ/サイズ上限を考慮）、結果は一覧に出してEnterでジャンプ。要件未登録なので新規 FR 化を検討（検索 UI・パフォーマンス・キャンセル） #feature #search
