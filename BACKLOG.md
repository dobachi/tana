...
- [ ] 2026-07-27 FR-07 続き: クラウド同期(OneDrive/Box)フォルダと WSL ディストロ(\\wsl$)の検出、Places の手動追加/永続化。第1スライス(ドライブ+標準フォルダ)は placesview.js/places.rs で実装済み #M2
...
- [ ] 2026-07-27 [FR-16] プレビュー: 画像が横幅いっぱいで表示され縦スクロールが必要。画像はペインに収まるよう縦横フィット(object-fit:contain / max-height:100%)にして、下配置なら縦幅基準で全体が見えるようにする。preview.js / previewkind.js / style.css の画像表示周り #ux #preview
- [ ] 2026-07-27 [FR-16] プレビュー: 画像の拡大縮小(ズーム)に未対応。Ctrl+ホイール/+-キー/フィット切替(実寸⇔ウィンドウフィット)などでズームできるようにする。パン(ドラッグ移動)も検討 #preview
